...
RBPRect MaxRectsBinPack::Insert(int width, int height, FreeRectChoiceHeuristic method)
{
	RBPRect newNode;
...
void MaxRectsBinPack::Insert(std::vector<RBPRectSize> &rects, std::vector<RBPRect> &dst, FreeRectChoiceHeuristic method)
{
	dst.clear();
	...
	while(rects.size() > 0)
	{
		...
		for(size_t i = 0; i < rects.size(); ++i)
		{
			int score1;
			int score2;
			RBPRect newNode = ScoreRect(rects[i].width, rects[i].height, method, score1, score2);
...
void MaxRectsBinPack::PlaceRect(const RBPRect &node)
{
	size_t numRectanglesToProcess = freeRectangles.size();
	for(size_t i = 0; i < numRectanglesToProcess; ++i)
	{
...
RBPRect MaxRectsBinPack::ScoreRect(int width, int height, FreeRectChoiceHeuristic method, int &score1, int &score2) const
{
	RBPRect newNode;
...
RBPRect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX) const
{
	RBPRect bestNode;
	memset(&bestNode, 0, sizeof(RBPRect));
...
RBPRect MaxRectsBinPack::FindPositionForNewNodeBestShortSideFit(int width, int height, 
...
RBPRect MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(int width, int height, 
	int &bestShortSideFit, int &bestLongSideFit) const
{
	RBPRect bestNode;
	memset(&bestNode, 0, sizeof(RBPRect));
...
RBPRect MaxRectsBinPack::FindPositionForNewNodeBestAreaFit(int width, int height, 
...
RBPRect MaxRectsBinPack::FindPositionForNewNodeContactPoint(int width, int height, int &bestContactScore) const
{
	RBPRect bestNode;
	memset(&bestNode, 0, sizeof(RBPRect));
...
bool MaxRectsBinPack::SplitFreeNode(RBPRect freeNode, const RBPRect &usedNode)
{
